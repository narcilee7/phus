...
> Historical snapshot based on `documents/Intelligence-Alignment.md` and `documents/Proposal-Monorepo-Split.md`; those source documents are not present in this checkout. Each row says what's landed, what's stub, and what's not started. Last audit: 2026-07-20.
...
| Channel address normalization & TUI Session UI              | ✓ Phase 4           | `Envelope.address` routes through `SessionStore.resolveOrCreate` for CLI, Telegram, Slack, WebSocket, SSE, and Email. TUI `/sessions`, `/use`, `/close`, `/reopen`, `/archive`, `/end` operate on catalog entities; sidebar shows status/address/last-turn. CLI gains `--session` plus Session-aware `resume`/`tasks`.                                                                                                        |
| Cross-channel identity, email thread collapse, hook context | ✓ Phase 5           | `SessionIdentityStore` maps `(channel, subjectId)` to a durable identity row. Email `buildAddress` collapses reply chains via References/In-Reply-To while preserving `metadata.messageId`. `HookContext.session?: SessionContextLike` carries the resolved Session to plugin hooks. CLI/Telegram/Slack/Email record their subject on every turn. Cross-channel merge UI and the full internal hook migration remain Phase 6. |
...
A completed design is not a shipped feature. Do not mark the Session-first refactor complete until its implementation phases and acceptance tests land.
...
| Prompt assembly pulls only the most relevant memory blocks | △     | Wired in `bridge/prompt-assembly.ts`, but retrieval ranking by _recency_ + _authority_ is partial — section-level scoring exists, query→section matching is shallow.                                                                                                                           |
